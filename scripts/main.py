...
logger.info(f"机器: {platform.node()} | Python {platform.python_version()} | pandas {pd.__version__}")
logger.info(f"渠道: {args.channel} | 报告: {args.report} | 月份: {args.month}")
...
try:
    ...
    if args.send:
        logger.info("===== 发送报告 =====")
        from send_report import summary_report, send_email, send_wechat
